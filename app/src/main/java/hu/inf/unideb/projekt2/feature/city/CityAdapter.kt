...
import hu.inf.unideb.projekt2.R
import hu.inf.unideb.projekt2.databinding.ItemCityBinding

class CityAdapter(private val listener: OnCitySelectedListener) :
    RecyclerView.Adapter<CityAdapter.CityViewHolder>() {
    private val cities: MutableList<String> = ArrayList()

    interface OnCitySelectedListener {
        fun onCitySelected(city: String?)
        fun onCityDeleted(position: Int)
    }
    ...
    inner class CityViewHolder(private val itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        init {
            ...
            binding.CityItemRemoveButton.setOnClickListener {
                listener.onCityDeleted(position)
            }
